...
class ClassificacaoAHP:
    ...
    def matriz_preferencia_preco(self):
        ...
        for i in range(self.n):
            for j in range(i + 1, self.n):
                ...
                nota = 1 if dif_max == 0 else 1 + 8 * (1 - dif / dif_max) #inversamente proporcional
                matriz[i, j] = nota
                matriz[j, i] = 1 / nota
    ...
    def matriz_preferencia_metro(self):
        metros = self.df["Area"].values
        matriz = np.ones((self.n, self.n))
        dif_max = np.max(np.abs(metros[:, None] - metros[None, :]))

        for i in range(self.n):
            for j in range(i + 1, self.n):
                dif = abs(metros[i] - metros[j])
                nota = 1 if dif_max == 0 else 1 + 8 * (dif / dif_max)#diretamente proporcional
                matriz[i, j] = nota
                matriz[j, i] = 1 / nota

        matriz_metros = matriz / matriz.sum(axis=0) # matriz normalizada Passo 2
        vetormedia_metros = matriz_metros.sum(axis=1) / len(matriz_metros) # média de cada linha

        print("Coluna media metros quadrados: \n",  vetormedia_metros)

        return vetormedia_metros
    ...
    def pontuacao_final(self):
        ...
        self.df["Pontuação Area"] = vetor_metros
